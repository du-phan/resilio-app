...
from resilio.schemas.sync import ActivitySyncState, SyncPhase
...
def _save_publication(repo: RepositoryIO, publication: PublishedWorkout) -> None:
    save_manifest(
        repo,
        PublicationManifest(
            workouts={publication.local_workout_id: publication}
        ),
    )
...
def test_immediate_repeat_sync_creates_zero_duplicates(
    ...
    repo = RepositoryIO()
    config = _config()
    metrics_calls: list[tuple[date, date]] = []

    def metrics(_repo, start, end):
        metrics_calls.append((start, end))
        return {}
    ...
    assert metrics_calls == [(date(2026, 7, 28), date(2026, 7, 28))]
...
def test_exact_paired_event_links_completion_idempotently(
    ...
    client.row = client.row.model_copy(
        update={"paired_event_id": publication.event_id}
    )
...
def test_unique_unpaired_candidate_is_reported_without_automatic_link(
    ...
    report = ActivitySyncService(repo, _config(), SyncClient()).run(
        today=date(2026, 7, 28)
    )
    ...
    artifact = json.loads(
        next(
            (tmp_path / "data" / "state" / "sync-runs").rglob(
                "quarantine.json"
            )
        ).read_text()
    )
    assert artifact["completion_candidates"] == [
        {
            "local_activity_id": (
                ActivityArchive(
                    tmp_path / "data" / "activities"
                ).load_all()[0].local_activity_id
            ),
...
def test_paired_event_sport_mismatch_fails_safe_without_completion_link(
    ...
    assert artifact["ambiguous_decisions"][0]["rule"] == (
...
def test_invalid_external_row_is_quarantined_without_archive_write(
    ...
    report = ActivitySyncService(repo, config, client).run(
        today=date(2026, 7, 28)
    )
...
def test_confirmed_404_tombstones_exact_linked_record(
    ...
    assert tombstone.calculated_load is None
...
def test_unconfirmed_404_is_exposed_in_local_deletion_review(
    ...
    assert report.partial
    assert report.activities_tombstoned == 0
    assert ActivityArchive(archive).load_all()[0].status == (
...
def test_metrics_failure_restores_archive_metrics_and_sync_state_then_retries(
    tmp_path,
    monkeypatch,
) -> None:
    (tmp_path / ".git").mkdir()
    archive = tmp_path / "data" / "activities"
    archive.mkdir(parents=True)
    metrics_root = tmp_path / "data" / "metrics"
    metrics_root.mkdir(parents=True)
    metric_snapshot = metrics_root / "snapshot.txt"
    metric_snapshot.write_text("before\n")
    monkeypatch.chdir(tmp_path)
    repo = RepositoryIO()
    write_sync_state(
        repo,
        ActivitySyncState(resolved_athlete_id="before-athlete"),
    )

    def fail_metrics(_repo, _start, _end):
        metric_snapshot.write_text("partially-recomputed\n")
    ...
    service = ActivitySyncService(
        ...
        _config(),
        SyncClient(),
        metrics_recompute=fail_metrics,
    )
    with pytest.raises(RuntimeError, match="simulated metrics failure"):
        service.run(today=date(2026, 7, 28))

    assert ActivityArchive(archive).load_all() == []
    assert metric_snapshot.read_text() == "before\n"
    assert read_sync_state(repo).resolved_athlete_id == "before-athlete"
    assert read_sync_progress(repo).phase == SyncPhase.FAILED

    service.metrics_recompute = lambda *_args: {}
    retried = service.run(today=date(2026, 7, 28))

    assert retried.activities_created == 1
...
def test_sync_state_failure_restores_workout_completion_manifest(
    ...
    original_manifest = WorkoutCompletionManifest(
        matches={old_match.local_activity_id: old_match}
    )
    ...
    with pytest.raises(RuntimeError, match="simulated state failure"):
        ActivitySyncService(repo, _config(), client).run(
            today=date(2026, 7, 28)
        )
...
def test_completion_save_revalidates_mutated_mapping(
    ...
    manifest = WorkoutCompletionManifest(
        matches={first.local_activity_id: first}
    )
    second = first.model_copy(
        update={"local_activity_id": "act_h_89abcdef0123456701234567"}
    )
...
def test_interrupted_archive_swap_is_recovered_before_retry(
    ...
    interrupted_archives = list(
        (tmp_path / "data" / "state" / "sync-runs").glob(
            "*/interrupted-archive"
        )
    )
