"""Canonical activity v4 and external mapping regression tests."""
...
def test_intervals_rpe_precedes_source_perceived_exertion() -> None:
    ...
    assert external_fingerprint(_external(icu_rpe=7)) != external_fingerprint(_external(icu_rpe=6))
...
def test_decimal_source_rpe_is_preserved_without_rounding() -> None:
    ...
    assert activity.subjective_effort is not None
    assert activity.subjective_effort.rpe_1_to_10 == 6.5


def test_current_mapper_records_canonical_mapping_version_seven() -> None:
    activity = map_activity(
        _external(),
        imported_at_utc=datetime(2026, 7, 28, tzinfo=timezone.utc),
    )

    assert activity.audit.canonical_mapping_version == 7
...
def test_interval_order_is_permutation_invariant() -> None:
    ...
    assert external_fingerprint(first) == external_fingerprint(reversed_source)
...
def test_absent_completion_pairing_does_not_change_existing_fingerprint() -> None:
    ...
    assert external_fingerprint(without_pairing) == external_fingerprint(explicit_null)
...
def test_literal_schema_alias_round_trips_and_legacy_shape_is_refused() -> None:
    ...
    assert payload["_schema"] == {"name": "resilio.activity", "version": 4}
