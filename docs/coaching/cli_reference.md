...
| **`sce plan create-macro`**             | Generate macro plan structure | phases, volume_trajectory, ctl_projections, recovery_weeks         |
| **`sce plan generate-month`**           | Generate monthly plan (2-6w)  | weeks with workouts, paces, generation_context                      |
| **`sce plan assess-month`**             | Assess completed month        | adherence_pct, ctl_delta, vdot_recalibration, recommendations      |
| **`sce plan validate-month`**           | Validate monthly plan (2-6w)  | overall_ok, violations, warnings, summary                           |
...
| **`sce vdot estimate-current`**         | Estimate VDOT from workouts  | estimated_vdot, confidence, supporting workout data                 |
| **`sce race add`**                      | Add race performance         | race details, VDOT, is_pb flag, peak_vdot updated                   |
| **`sce race list [--distance] [--since]`** | List race history         | races grouped by distance with VDOTs and PB flags                   |
| **`sce race import-from-strava`**       | Auto-detect races            | detected races from Strava (last 120 days) for confirmation         |
...
### Add Race Performance

#### `sce race add --distance --time --date [--location] [--source] [--notes]`
...
#### `sce race list [--distance DISTANCE] [--since DATE]`
...
### Import Races from Strava

#### `sce race import-from-strava [--since DATE] [--interactive]`
...
### Estimate Current VDOT (Enhanced VDOT Command)

#### `sce vdot estimate-current [--lookback-days DAYS]`
