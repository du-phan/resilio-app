...
**Tech Stack:** Markdown skills (`SKILL.md`), CLI commands (`resilio plan validate-week|validate-intervals|validate-structure`).
...
Add a sub-step under validation to run `resilio plan validate-intervals` **only** when the weekly plan includes a structured interval/tempo session with explicit work/recovery bouts. Include a short note on preparing `/tmp/work_bouts.json` and `/tmp/recovery_bouts.json` from the planned session.
...
After `resilio plan create-macro`, export the stored structure and validate:
